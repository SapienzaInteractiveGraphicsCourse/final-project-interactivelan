var e=class{constructor(){this.waveCounterElement=null,this.stateElement=null,this.crosshairElement=null,this.buildWaveCounter(),this.buildScore(),this.buildStateIndicator(),this.initCrosshair()}async initCrosshair(){try{let e=await(await fetch(`/final-project-interactivelan/ui/crosshair.svg`)).text();this.crosshairElement=document.createElement(`div`),this.crosshairElement.id=`crosshair-ui`,this.crosshairElement.style.cssText=`
        ...
        `,document.head.appendChild(e)}};function t(e){let t=document.createElement(`div`);t.style.cssText=[`position:fixed`,`bottom:16px`,`left:16px`,`font-family:monospace`,`font-size:12px`,`color:#ccc`,`background:rgba(0,0,0,0.5)`,`padding:7px 14px`,`border-radius:4px`,`pointer-events:none`,`line-height:1.85`,`z-index:9999`,`user-select:none`].join(`;`);let r=new Map;for(let[i,a]of e){let e=document.createElement(`div`);e.innerHTML=n(i,a),t.appendChild(e),r.set(i,e)}return document.body.appendChild(t),{setLabel(e,t){let i=r.get(e);i&&(i.innerHTML=n(e,t))}}}function n(e,t){return`<span style="color:#7bcfff;display:inline-block;min-width:76px">${e}</span>${t}`}export{t as n,e as t};